...
/// signal-backed `for` loops, and layout-only directives such as `$if` and
/// `$wrapper`.
///
/// ```ignore
/// layout! {
...
/// ```
///
/// `$wrapper` nests an element inside one or more wrapper components:
///
/// ```ignore
/// layout! {
///     Content($wrapper = [ThemeProvider(theme), StyleProvider(styles)])
/// }
/// ```
#[proc_macro]
